...
# this list (and order) of columns is used if $columns parameter is not specified
	...
	WriteLog('GetResultSetAsDialog: $title = ' . $title . '; $resultCount = ' . $resultCount . '; $columnsDisplay = ' . ($columnsDisplay ? $columnsDisplay : 'UNDEFINED'));
...
sub GetWindowTemplate2 { # \%paramHash ; returns window template
	...
	if (GetConfig('admin/js/enable') && GetConfig('admin/js/dragging')) {
		...
		$windowTemplate = AddAttributeToTag($windowTemplate, 'table', 'onmouseenter', 'if (window.SetActiveDialog) { return SetActiveDialog(this); }'); #SetActiveDialog
		$windowTemplate = AddAttributeToTag($windowTemplate, 'table', 'onmousedown', 'if (window.SetActiveDialog) { return SetActiveDialog(this); }'); #SetActiveDialog
	}
...
#				$row->{'show_quick_vote'} = 1;
	...
	if ($pageType eq 'author') {
		...
		$txtIndex = InjectJs($txtIndex, qw(itsyou settings timestamp voting utils profile fresh));
	} else {
		if ($needUploadJs) {
